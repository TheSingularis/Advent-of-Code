...
for game in input:
    ...
    matches = []
    while ";" in game:
        match_buffer, game = game.split(";", 1)
        matches.append(match_buffer)
    matches.append(game)
    for match in matches:
        match = match.strip()
        colors = []
        while "," in match:
            color, match = match.split(",", 1)
            colors.append(color)
        colors.append(match)
        for color in colors:
            color = color.strip()
            if "red" in color:
                if int(color.split(" ", 2)[0]) > red:
                    red = int(color.split(" ", 2)[0])
            if "green" in color:
                if int(color.split(" ", 2)[0]) > green:
                    green = int(color.split(" ", 2)[0])
            if "blue" in color:
                if int(color.split(" ", 2)[0]) > blue:
                    blue = int(color.split(" ", 2)[0])

    game_total = red * green * blue
    game_power.append(game_total)
    
total = 0
for num in game_power:
    total += int(num)
