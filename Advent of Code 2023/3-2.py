...
space = ["".join("." for i in range(len(input[0])))]
...
grid = [[0 for i in range(len(input[0]))] for j in range(len(input))]
part_nums = []
gear_ratios = []
x, y = 0, 0
for row in input:
            ...
            if has_gear is False:
                for i in range(-1, 2):
                    for j in range(-1, 2):
                        if i == 0 and j == 0:
                            continue
                        test_square = grid[max(
                            0, min(y+i, len(grid)-1))][max(0, min(x+j, len(row)-1))]
                        if test_square == "*":
                            has_gear = True
                            gear_pos = (x+j, y+i)
            if x == len(row)-1:
                if has_gear is True:
                    part_nums.append((buffer, gear_pos))
...
for a, num_a in enumerate(part_nums):
    num_a, gear_a = num_a[0], num_a[1]
    for b, num_b in enumerate(part_nums):
        if a <= b:
            continue

        num_b, gear_b = num_b[0], num_b[1]

        if gear_a == gear_b:
            gear_ratios.append(int(num_a) * int(num_b))
